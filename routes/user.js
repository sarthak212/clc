...
router.get('/admin/vendors', restrict, async (req, res) => {
    const db = req.app.db;
    const vendors = await db.supervisors.find({}, { projection: { userPassword: 0 } }).toArray();
...
router.get('/admin/vendor/edit/:id', restrict, async (req, res) => {
    const db = req.app.db;
    const vendor = await db.supervisors.findOne({ _id: common.getId(req.params.id) });
...
router.post('/admin/vendor/delete', restrict, async (req, res) => {
    ...
    try{
        await db.supervisors.deleteOne({ _id: common.getId(req.body.vendorId) }, {});
        res.status(200).json({ message: 'User deleted.' });
        return;
    }catch(ex){
        console.log('Failed to delete user', ex);
...
router.post('/admin/vendor/update', restrict, async (req, res) => {
  ...
  //  updateDoc.isAdmin = isAdmin;
    if(req.body.vendorName){
        updateDoc.userName = req.body.vendorName;
    }
    if(req.body.vendorEmail){
        updateDoc.userEmail = req.body.vendorEmail;
    }
    if(req.body.vendorAadhar){
        updateDoc.userAadhar = req.body.vendorAadhar;
    }
    if(req.body.vendorPhone){
        updateDoc.userPhone = req.body.vendorPhone;
    }
    if(req.body.userPassword){
        updateDoc.userPassword = bcrypt.hashSync(req.body.vendorPassword);
    }
   ...
   /* if(!schemaResult.result){
    ...
    try{
        const updatedUser = await db.supervisors.findOneAndUpdate(
        ...
        delete returnUser.userPassword;
        delete returnUser.apiKey;
        res.status(200).json({ message: 'User account updated', vendor: updatedUser.value });
        return;
    }catch(ex){
        console.error(colors.red('Failed updating user: ' + ex));
        res.status(400).json({ message: 'Failed to update user' });
...
router.post('/admin/vendor/insert', restrict, async (req, res) => {
    ...
    const userObj = {
        ...
        userAadhar: req.body.vendorAadhar,
        userPhone: req.body.vendorPhone,
        userPassword: bcrypt.hashSync(req.body.vendorPassword, 10)
    };
   ...
   /* const schemaResult = validateJson('newUser', userObj);
    ...
    // check for existing user
    const user = await db.supervisors.findOne({ vendorEmail: req.body.vendorEmail });
    if(user){
        console.error(colors.red('Failed to insert user, possibly already exists'));
        res.status(400).json({ message: 'A user with that email address already exists' });
        return;
    ...
    // email is ok to be used.
    try{
        const newUser = await db.supervisors.insertOne(userObj);
        res.status(200).json({
            message: 'User account inserted',
            userId: newUser.insertedId
        });
